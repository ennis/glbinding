...
def forwardBitfield(api, enum):
    ...
    if len(enum.groups) == 1:
        ...
        return "static const %sbinding::SharedBitfield<%s> %s = %s::%s;" % (api, groups, enumBID(enum), group, enumBID(enum))



def bitfieldGroup(api, group, enums, usedBitfsByName):
    ...
    spaces = " " * (maxlen - len(prefix + "_NONE_BIT"))
    enumGroupString.insert(0, "    " + prefix + "_NONE_BIT " + spaces + "= 0x0, // Generic " + prefix + "_NONE_BIT")
...
def genFeatureBitfields(api, enums, feature, outputdir, outputfile, core = False, ext = False):
    ...
    pureBitfields    = [ b for b in tgrouped[prefix + "bitfield"] if 
    ...
    if feature:
        ...
        if len(importToNamespace):
            importToNamespace.insert(0, "using " + api + "::" + prefix + "_NONE_BIT;")
    else:
        groups = ", ".join([ api + "::" + g for g in sorted(groupedBitfields.keys()) ])
        importToNamespace = [ forwardBitfield(api, e) for e in sorted(pureBitfields) ]
        if len(importToNamespace):
            importToNamespace.insert(0, "static const %sbinding::SharedBitfield<%s> %s_NONE_BIT = %s::%s::%s_NONE_BIT;" 
                % (api, groups, prefix, api, sorted(groupedBitfields.keys())[0], prefix))
    ...
    with open(od + of, 'w') as file:
        ...
        if not feature:
            ...
            definitions = [ bitfieldGroup(api, group, enums, usedBitfsByName) 
