...
def genFeature(api, feature, outputdir, outputfile, core = False, ext = False):
    ...
    include = "\n#include <" + api + "binding/" + api + versionExtDir + "/%s" + versionExtFile + ".h>"
    ...
    with open(od + of, 'w') as file:
        if core or ext or (not feature) or (api != "gl"):
            ...
            file.write(t.replace("%f", version) % (
                include.replace("%s", "types"),
                include.replace("%s", "boolean"),
                include.replace("%s", "values"),
                include.replace("%s", "bitfield"),
                include.replace("%s", "enum"),
                include.replace("%s", "functions")))
