...
def typeImport(api, type):
    ...
    if type.name[len(api):] in enum_classes:
        return "using " + api + "::" + type.name[0:len(api)] + type.name[len(api):].lower() + ";" # EGLBoolean <-> GLboolean
...
def genFeatureTypes(api, types, bitfGroups, feature, outputdir, outputfile, core = False, ext = False):
    ...
    with open(od + of, 'w') as file:
        ...
        file.write(t %
            ...
            "\n".join([ "using %s::%s;" % (api, g.name) for g in bitfGroups ]),)
        )
...
def genTypeIntegrations_h(api, types, bitfGroups, outputdir, outputfile):
    ...
    with open(od + of, 'w') as file:
        ...
        for typename, integrations in type_integration_map.items():
            for integration in integrations:
                type_integrations.append(template("type_integration/%s.h" % integration).replace(
                    "%a", api).replace("%A", api.upper()).replace("%t", prefix + typename))

        for group in bitfGroups:
            for integration in [ "hashable", "bitfield_streamable", "bit_operatable"]:
                type_integrations.append(template("type_integration/%s.h" % integration).replace(
                    "%a", api).replace("%A", api.upper()).replace("%t", group.name))

        file.write(t % ("\n".join([ t for t in type_integrations ])))


def genTypeIntegrations_cpp(api, types, bitfGroups, outputdir, outputfile):

    prefix = api.upper()

    of = outputfile.replace("?", "")
    od = outputdir.replace("?", "")
    t = template(of).replace("%a", api).replace("%A", prefix)
    ...
    for typename, integrations in type_integration_map.items():
        for integration in integrations:
            type_integrations.append(template("type_integration/%s.cpp" % integration).replace(
                "%a", api).replace("%A", prefix).replace("%t", prefix + typename)) 

    for group in bitfGroups:
        for integration in [ "hashable", "bitfield_streamable", "bit_operatable"]:
            type_integrations.append(template("type_integration/%s.cpp" % integration).replace(
                "%a", api).replace("%A", prefix).replace("%t", group.name))
